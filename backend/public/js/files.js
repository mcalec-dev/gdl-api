  ...
  for (const item of sortedContents) {
    ...
    if (itemType === 'directory') {
      if (hasDirectories && !hasFiles) {
        html += `
        <div class="file-item directory group bg-black/80 flex items-center h-auto w-full p-4 m-0 border border-white/20 rounded-xl text-white pointer-events-auto box-border overflow-hidden select-none ${cursorClass}" data-type="${itemType}" data-file-type="${itemType}" data-path="${itemPath}" data-size="${item.size}" ${item.uuid ? `data-uuid="${item.uuid}"` : ''} ${item.hash ? `data-hash="${item.hash}"` : ''}>
      ...
      `
        ...
        html += `
        <div class="file-item directory group place-items-start bg-black/80 flex-row h-auto w-full p-0 m-0 border border-white/20 rounded-xl text-white pointer-events-auto box-border overflow-hidden select-none ${cursorClass}" data-type="${itemType}" data-file-type="${itemType}" data-path="${itemPath}" data-size="${item.size}" ${item.uuid ? `data-uuid="${item.uuid}"` : ''} ${item.hash ? `data-hash="${item.hash}"` : ''}>
      ...
      html += `
        <div class="file-item ${item.type} group relative bg-black/80 block w-full h-full align-middle items-center justify-center p-0 break-inside-avoid border border-white/20 rounded-xl text-white pointer-events-auto box-border overflow-hidden select-none ${cursorClass}" data-type="${itemType}" data-file-type="${itemType}" data-path="${itemPath}" data-size="${item.size}" ${item.uuid ? `data-uuid="${item.uuid}"` : ''} ${item.hash ? `data-hash="${item.hash}"` : ''}>
      ...
      html += `
        <div class="file-item other group bg-black/80 relative flex w-full h-auto p-3 border border-white/20 rounded-xl text-white pointer-events-auto box-border overflow-hidden select-none ${cursorClass}" data-type="${itemType}" data-file-type="${itemType}" data-path="${itemPath}" data-size="${item.size || 0}" ${item.uuid ? `data-uuid="${item.uuid}"` : ''} ${item.hash ? `data-hash="${item.hash}"` : ''}>
